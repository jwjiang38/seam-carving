...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        double [][] acc= new double [m.length][m[0].length];
        ...
        for (int i=0;i<m.length;i++){
            for (int j=0;j<m[0].length;j++){
                if (i==0) acc[i][j]=m[i][j];
                else {
                    acc[i][j]=m[i][j]+Double.min(Double.min(acc[i-1][Integer.max(j-1,0)],acc[i-1][j]),acc[i-1][Integer.min(j+1,m[0].length-1)]);
                }
            }
        }
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        if (orientation == Orientation.VERTICAL) return accumulateVertical(m);
        else {
            ...
            double[][] np = accumulateVertical(n);
            double[][] nout = new double[m.length][m[0].length];
            for (int i=0;i<m.length;i++){
                for (int j=0;j<m[0].length;j++){
                    nout[i][j]=np[j][i];
                }
    ...
    public static int[] findVerticalSeam(double[][] m) {
        ...
        for (int j=0;j<m[0].length;j++){
            if (m[m.length-1][j]<minn) {
                index[index.length-1]=j;
                minn = m[m.length-1][j];
            }
        }
        for (int i=m.length-2;i>=0;i--){
            double curmin = m[i][index[i+1]];
            index[i]=index[i+1];
            for (int j=Integer.max(index[i+1]-1,0);j<=Integer.min(index[i+1]+1,m[0].length-1);j++){
                if (m[i][j]<curmin){
                    curmin=m[i][j];
                    index[i]=j;
                }
    ...
    public static int[] findSeam(double[][] m, Orientation orientation) {
        if (orientation == Orientation.VERTICAL) return findVerticalSeam(m);
        else {
            double[][] n = new double[m[0].length][m.length];
            for (int i=0;i<m.length;i++){
                for (int j=0;j<m[0].length;j++){
